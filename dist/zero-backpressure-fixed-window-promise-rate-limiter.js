...
class FixedWindowRateLimiter {
     ...
     */
    constructor(windowDurationMs, maxStartsPerWindow) {
        this._onWindowEndHandler = (notifyWindowEnded) => {
            this._currWindowStartsCounter = 0;
            notifyWindowEnded();
        };
        this._waitForWindowToEnd = Promise.resolve();
        this._currWindowStartsCounter = 0; // The number of tasks that started during the current window.
        ...
        if (windowDurationMs < MIN_ALLOWED_WINDOW_DURATION_MS) {
            ...
            throw new Error(`FixedWindowRateLimiter instantiation failed: windowDurationMs value of ` +
        ...
        if (!isNaturalNumber(maxStartsPerWindow)) {
            throw new Error(`FixedWindowRateLimiter instantiation failed: maxStartsPerWindow value of ` +
        ...
        this._availableSlotsStack = new Array(initialSlotsCount).fill(0);
     ...
     */
    get amountOfCurrentlyExecutingTasks() {
     ...
     */
    get amountOfTasksInitiatedDuringCurrentWindow() {
     ...
     */
    async startExecution(backgroundTask) {
     ...
     */
    async waitForAllExecutingTasksToComplete() {
    ...
    async _allotExecutionSlot() {
        ...
        if (++this._currWindowStartsCounter === 1) {
            ...
            this._waitForWindowToEnd = new Promise(notifyWindowEnded => setTimeout(this._onWindowEndHandler, this._windowDurationMs, notifyWindowEnded));
        }
     ...
     */
    async _handleTaskExecution(task, allottedSlot, isBackgroundTask) {
        try {
...
//# sourceMappingURL=zero-backpressure-fixed-window-promise-rate-limiter.js.map