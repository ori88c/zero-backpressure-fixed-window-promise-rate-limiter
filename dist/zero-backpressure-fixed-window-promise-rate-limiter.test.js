...
const zero_backpressure_fixed_window_promise_rate_limiter_1 = require("./zero-backpressure-fixed-window-promise-rate-limiter");
/**
 * resolveFast
 *
...
describe('FixedWindowRateLimiter tests', () => {
    ...
    const triggerEndingOfCurrentWindow = () => {
    ...
    describe('Happy path tests', () => {
        ...
        test('when the window capacity is exhausted, the Rate Limiter should stop executing tasks utill a new window opens up', async () => {
            ...
            const finishTaskCallbacks = [];
            const createTask = () => new Promise(res => finishTaskCallbacks.push(res));
    ...
    describe('Negative path tests', () => {
        test('should throw if window duration is less than 15ms or non-natural number', async () => {
            ...
            for (const windowDurationMs of invalidWindowDurations) {
                expect(() => new zero_backpressure_fixed_window_promise_rate_limiter_1.FixedWindowRateLimiter(windowDurationMs, MOCK_MAX_STARTS_PER_WINDOW)).toThrow();
            }
        });
        test('should throw if max starts per window is a non-natural number', async () => {
            ...
            for (const maxStartsPerWindow of invalidMaxStartsPerWindow) {
                expect(() => new zero_backpressure_fixed_window_promise_rate_limiter_1.FixedWindowRateLimiter(MOCK_WINDOW_DURATION_MS, maxStartsPerWindow)).toThrow();
            }
        });
...
//# sourceMappingURL=zero-backpressure-fixed-window-promise-rate-limiter.test.js.map